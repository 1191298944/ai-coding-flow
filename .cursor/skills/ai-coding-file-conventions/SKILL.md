...
| 产物 | 路径 |
|---|---|
| 需求上下文 | `~/.cursor/.llm/<业务名>/context.md` |
...
- `context.md`：「当前需求」与「接口契约」节覆盖写入，「变更历史」节追加（行数只增不减）
...
| 操作 | 范围 |
|---|---|
| 允许读 | 用户需求原文；`context.md`（修订时必读，作为本轮起点）；数据库 schema |
| 禁止读 | `src/` 下任何业务代码；spec-writer / implementor / test-writer 产物 |
| 允许写 | `~/.cursor/.llm/<业务名>/context.md` |
...
| 操作 | 范围 |
|---|---|
| 允许读 | `context.md` 的「当前需求」节（路径见上方存储路径表）；数据库 schema；业务样本数据；其他需求文档（仅作结构参考） |
| 禁止读 | `src/main/java/` 下任何业务代码；implementor / test-writer 产物 |
...
| 操作 | 范围 |
|---|---|
| 允许读 | `context.md` 的「当前需求」节（路径见上方存储路径表）；`src/` 下已有业务代码；数据库 schema；项目规则 |
| 禁止读 | `需求文档.md`（spec-writer 产物）；测试模块下任何文件 |
...
| 禁止读 | `context.md`；implementor 产出的 Controller 方法体 / Service / Mapper / XML 内部逻辑 |
